...
const formatDeviceInfo = (device) => {
  ...
  return `${device.browser?.name || 'Unknown browser'} on ${device.os?.name || 'Unknown OS'}`;
};
  ...
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          id="lastLogin"
          value={user.lastLogin ? new Date(user.lastLogin.date).toLocaleString() : 'Never'}
          readOnly
        />
      </div>
    </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">
                        {session.logoutTime ? 'Past Session' : 'Current Session'}
                      </p>
                      <p className="text-sm text-muted-foreground">
...
export function UserDetailDrawer({ user, open, onClose }) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await axiosInstance.get(`/auth/sessions?userId=${user?._id}`);
      setSessions(response.data);
    } catch (err) {
      setError(prev => ({ ...prev, sessions: err instanceof Error ? err.message : 'Unknown error' }));
    } finally {
  ...
  const fetchLoginHistory = async () => {
    try {
      ...
      const response = await axiosInstance.get(`/auth/login-history?userId=${user?._id}`);
      console.log(response);
      setLoginHistory(response.data);
    } catch (err) {
      setError(prev => ({ ...prev, loginHistory: err instanceof Error ? err.message : 'Unknown error' }));
    } finally {
